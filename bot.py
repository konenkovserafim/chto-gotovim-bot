...
def recipe_context(recipe: dict[str, Any]) -> str:
...
def detailed_steps(recipe: dict[str, Any]) -> list[str]:
    """Возвращает более подробное приготовление, если в базе короткая заглушка."""
    original_steps = [str(step).strip() for step in recipe.get("steps", []) if str(step).strip()]
    original_size = sum(len(step) for step in original_steps)
    if len(original_steps) >= 5 and original_size >= 220:
        return original_steps

    ctx = recipe_context(recipe)

    if any(word in ctx for word in ["паста", "макарон", "спагетти", "болоньез"]):
        return [
            "Подготовьте ингредиенты: мелко нарежьте лук, при необходимости измельчите чеснок и достаньте соус заранее.",
            "Поставьте кастрюлю с водой на сильный огонь, посолите и доведите до кипения.",
            "Отварите пасту до состояния al dente по инструкции на упаковке. Перед сливом оставьте немного воды от пасты.",
            "Разогрейте сковороду, добавьте немного масла и обжарьте лук 3–4 минуты до мягкости.",
            "Добавьте мясо, курицу, фарш или другие основные ингредиенты и готовьте, пока они не станут полностью готовы.",
            "Влейте соус или добавьте овощи, перемешайте и прогрейте 5–10 минут на среднем огне.",
            "Соедините пасту с соусом. Если получилось густо, добавьте немного воды от пасты и перемешайте.",
            "Попробуйте на соль и специи, затем подавайте горячей. По желанию добавьте сыр или зелень.",
        ]

    if "суп" in ctx or any(word in ctx for word in ["борщ", "щи", "харчо"]):
        return [
            "Подготовьте продукты: мясо или птицу промойте, овощи очистите и нарежьте удобными кусочками.",
            "Если используется мясо, залейте его водой, доведите до кипения и снимите пену.",
            "Добавьте ингредиенты, которые готовятся дольше всего: картофель, крупу, капусту или бобовые.",
            "Отдельно обжарьте лук и морковь 4–5 минут до мягкости, затем добавьте их в суп.",
            "Варите на среднем огне до готовности всех ингредиентов, периодически помешивая.",
            "Добавьте соль, специи и зелень ближе к концу приготовления.",
            "Дайте супу постоять под крышкой 10 минут, чтобы вкус стал насыщеннее.",
        ]

    if "салат" in ctx or any(word in ctx for word in ["цезарь", "греческий"]):
        return [
            "Подготовьте все ингредиенты: овощи вымойте и обсушите, мясо или рыбу заранее приготовьте и остудите.",
            "Нарежьте овощи и остальные продукты кусочками примерно одинакового размера.",
            "Если в рецепте есть курица, рыба или яйца, добавьте их в салат после остывания, чтобы листья и овощи не размякли.",
            "Смешайте ингредиенты в большой миске, но не давите их, чтобы салат сохранил текстуру.",
            "Добавьте заправку перед подачей и аккуратно перемешайте.",
            "Попробуйте на соль и подавайте сразу, пока овощи свежие и хрустящие.",
        ]

    if any(word in ctx for word in ["овсян", "каша", "рисовая каша", "гречневая каша", "манная"]):
        return [
            "Подготовьте крупу: при необходимости промойте её и уберите лишний мусор.",
            "В кастрюле соедините крупу с молоком, водой или их смесью, добавьте щепотку соли.",
            "Доведите до кипения на среднем огне, затем уменьшите нагрев.",
            "Готовьте, периодически помешивая, пока каша не станет мягкой и кремовой.",
            "Если каша слишком густая, добавьте немного молока или воды и прогрейте ещё минуту.",
            "Снимите с огня, дайте постоять под крышкой 3–5 минут.",
            "Перед подачей добавьте мёд, фрукты, ягоды, орехи или кусочек сливочного масла.",
        ]

    if any(word in ctx for word in ["омлет", "яичниц", "яйца"]):
        return [
            "Подготовьте начинку: нарежьте овощи, сыр, зелень или мясные продукты, если они используются.",
            "В миске взбейте яйца с щепоткой соли. Для более нежной текстуры можно добавить немного молока.",
            "Разогрейте сковороду на среднем огне и слегка смажьте маслом.",
            "Если есть овощи или мясная начинка, обжарьте её 2–4 минуты до мягкости.",
            "Влейте яичную смесь, уменьшите огонь и готовьте под крышкой до схватывания.",
            "Добавьте сыр или зелень ближе к концу, чтобы они не пересохли.",
            "Подавайте сразу, пока омлет или яичница горячие и нежные.",
        ]

    if "сырник" in ctx:
        return [
            "Разомните творог вилкой или протрите через сито, если хотите более нежную текстуру.",
            "Добавьте яйцо, немного сахара и муку, затем перемешайте до мягкого теста.",
            "Оставьте массу на 5–10 минут, чтобы мука впитала лишнюю влагу.",
            "Сформируйте сырники влажными руками и слегка обваляйте их в муке.",
            "Разогрейте сковороду с небольшим количеством масла.",
            "Обжарьте сырники по 3–4 минуты с каждой стороны до золотистой корочки.",
            "Подавайте со сметаной, йогуртом, ягодами или мёдом.",
        ]

    if any(word in ctx for word in ["рыба", "лосос", "треск", "минтай", "форель"]):
        return [
            "Промойте рыбу и обсушите бумажным полотенцем, чтобы она лучше запекалась или обжаривалась.",
            "Посолите, добавьте специи и немного лимонного сока, если он есть.",
            "Оставьте рыбу на 10 минут, чтобы она слегка промариновалась.",
            "Приготовьте гарнир или овощи, которые будут подаваться вместе с рыбой.",
            "Запеките рыбу в духовке или обжарьте на среднем огне до готовности.",
            "Не передерживайте рыбу: она должна остаться сочной.",
            "Подавайте горячей с овощами, картофелем, рисом или лёгким салатом.",
        ]

    if any(word in ctx for word in ["куриц", "индейк", "фарш", "говядин", "котлет", "рис", "греч", "картоф"]):
        return [
            "Подготовьте ингредиенты: мясо или птицу нарежьте, овощи очистите, крупу при необходимости промойте.",
            "Разогрейте сковороду или кастрюлю с небольшим количеством масла.",
            "Обжарьте лук, морковь или другие овощи 3–5 минут до мягкости, если они есть в рецепте.",
            "Добавьте основной белковый ингредиент: курицу, индейку, фарш, рыбу или мясо. Готовьте до изменения цвета и лёгкой корочки.",
            "Добавьте гарнир, соус, воду или бульон по рецепту и перемешайте.",
            "Готовьте на среднем огне до мягкости крупы, картофеля или овощей и полной готовности мяса.",
            "В конце попробуйте блюдо на соль и специи, при необходимости скорректируйте вкус.",
            "Дайте блюду постоять 5 минут под крышкой и подавайте горячим.",
        ]

    return original_steps or [
        "Подготовьте все ингредиенты и заранее достаньте продукты, которые должны быть комнатной температуры.",
        "Нарежьте, промойте или измельчите продукты так, как удобно для дальнейшего приготовления.",
        "Разогрейте сковороду, кастрюлю или духовку в зависимости от рецепта.",
        "Готовьте основные ингредиенты до полной готовности, периодически помешивая.",
        "Добавьте соль, специи, соус или зелень ближе к концу приготовления.",
        "Попробуйте блюдо, доведите вкус до нужного состояния и подавайте тёплым.",
    ]
...
def recipe_tips(recipe: dict[str, Any]) -> list[str]:
    ctx = recipe_context(recipe)
    tips = []
    if any(word in ctx for word in ["паста", "макарон", "спагетти"]):
        tips.extend([
            "Не переваривайте пасту: лучше оставить её слегка плотной.",
            "Немного воды от пасты помогает сделать соус более шелковистым.",
        ])
    elif "салат" in ctx:
        tips.extend([
            "Заправляйте салат перед подачей, чтобы овощи не дали лишний сок.",
            "Если добавляете мясо или яйца, остудите их перед смешиванием.",
        ])
    elif any(word in ctx for word in ["суп", "борщ", "щи"]):
        tips.extend([
            "Суп станет вкуснее, если дать ему настояться 10–15 минут.",
            "Зелень лучше добавлять в конце, чтобы сохранить аромат.",
        ])
    elif any(word in ctx for word in ["каша", "овсян"]):
        tips.extend([
            "Для более нежной текстуры готовьте кашу на смеси молока и воды.",
            "Сладкие добавки лучше класть в конце, чтобы вкус был ярче.",
        ])
    else:
        tips.extend([
            "Готовьте на среднем огне: так блюдо лучше прогреется и не подгорит.",
            "Пробуйте блюдо в конце и регулируйте соль и специи под свой вкус.",
        ])
    return tips[:3]
...
def recipe_storage(recipe: dict[str, Any]) -> list[str]:
    ...
    if "салат" in ctx:
        return [
            "Лучше съесть сразу после приготовления.",
            "Без заправки можно хранить в холодильнике до 1 суток.",
        ]
    if any(word in ctx for word in ["рыба", "лосос", "треск", "минтай"]):
        return [
            "В холодильнике — до 1–2 суток.",
            "Разогревать лучше аккуратно, чтобы рыба не стала сухой.",
        ]
    if any(word in ctx for word in ["суп", "борщ", "щи"]):
        return [
            "В холодильнике — до 3 суток.",
            "На следующий день вкус часто становится насыщеннее.",
        ]
    return [
        "В холодильнике — до 2–3 суток.",
        "Разогревать лучше на сковороде или в микроволновке небольшими порциями.",
    ]


def recipe_substitutions(recipe: dict[str, Any]) -> list[str]:
    ctx = recipe_context(recipe)
    substitutions = []
    if "фарш" in ctx:
        substitutions.append("Фарш можно заменить курицей, индейкой или грибами.")
    if "куриц" in ctx:
        substitutions.append("Курицу можно заменить индейкой или нежирной говядиной.")
    if "сметан" in ctx:
        substitutions.append("Сметану можно заменить греческим йогуртом.")
    if "сливк" in ctx:
        substitutions.append("Сливки можно заменить молоком с небольшим количеством сыра или сметаны.")
    if "рис" in ctx:
        substitutions.append("Рис можно заменить гречкой, булгуром или макаронами.")
    if "паста" in ctx or "макарон" in ctx:
        substitutions.append("Пасту можно заменить макаронами из твёрдых сортов пшеницы или цельнозерновыми.")
    if not substitutions:
        substitutions.append("Основные ингредиенты можно менять на похожие по вкусу и текстуре.")
    return substitutions[:3]


def recipe_serving(recipe: dict[str, Any]) -> list[str]:
    ctx = recipe_context(recipe)
    if any(word in ctx for word in ["паста", "макарон", "рис", "греч", "картоф"]):
        return ["Хорошо сочетается с овощным салатом.", "Можно подать с зеленью или лёгким соусом."]
    if "салат" in ctx:
        return ["Можно подать как лёгкий ужин или дополнение к основному блюду.", "Хорошо сочетается с хлебом, тостами или лавашом."]
    if any(word in ctx for word in ["суп", "борщ", "щи"]):
        return ["Подавать горячим.", "Хорошо сочетается со сметаной, зеленью или хлебом."]
    return ["Подавайте горячим или тёплым.", "Можно добавить свежие овощи или зелень."]


def format_recipe(r: dict[str, Any]) -> str:
    ingredients = "\n".join(f"• {escape(str(item))}" for item in r.get("ingredients", []))
    steps = "\n".join(f"{i}. {escape(step)}" for i, step in enumerate(detailed_steps(r), 1))
    tips = "\n".join(f"• {escape(tip)}" for tip in recipe_tips(r))
    storage = "\n".join(f"• {escape(item)}" for item in recipe_storage(r))
    substitutions = "\n".join(f"• {escape(item)}" for item in recipe_substitutions(r))
    serving = "\n".join(f"• {escape(item)}" for item in recipe_serving(r))
    tags = ", ".join(r.get("tags", []))

    blocks = [
        f"🍽 <b>{escape(str(r['name']))}</b>",
        f"⏱ <b>Время:</b> {r.get('time', '—')} мин\n"
        f"🔥 <b>Калорийность на двоих:</b> ~{r.get('calories', '—')} ккал\n"
        f"💰 <b>Примерно:</b> ~{r.get('price', '—')} ₽\n"
        f"🔎 <b>Сложность:</b> {escape(str(r.get('difficulty', 'Легко')))}",
        f"🛒 <b>Ингредиенты</b>\n{ingredients}",
        f"👨‍🍳 <b>Приготовление</b>\n{steps}",
        f"💡 <b>Советы</b>\n{tips}",
        f"📦 <b>Хранение</b>\n{storage}",
